...
```{r score propension sur inscription liste de greffe, include=FALSE}
iga %>%
  mutate(x = as.numeric(DINSCMED - DDIRT, units = "days")/365.25) %>% 
  filter(METHOn!="Greffe") %>% select(x) %>%  
  ggplot(aes(x)) + 
  geom_histogram(fill = "white", col = "black") +
  ggtitle("Durée entre suppléance et inscription liste greffe\n (droite = inscription post-suppléance)") +
  xlab("Temps (années)")
