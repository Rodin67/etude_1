...
gnc$nephgp <- ifelse(gnc$liste_longue=="Néphropathie lupique", gnc$nephgp <- "gnc2", gnc$nephgp <- gnc$nephgp)
...
library(mice)
modele.0.gr <- mice.tab.hd.gr %>% with(data=., exp = glm(METHOn ~ nephgp, family = "binomial")) %>% pool() %>% summary()

modele.1.gr <- mice.tab.hd.gr %>% with(data=., exp = glm(METHOn ~ nephgp+age+sex+anirt, family = "binomial")) %>% pool() %>% summary()

modele.2.gr <- mice.tab.hd.gr %>% with(data=., exp = glm(METHOn ~ nephgp+age+sex+anirt+DIABn+ICn+avc_ait+KCn+gr_bmi, family = "binomial")) %>% pool() %>% summary()

modele.3.gr <- mice.tab.hd.gr %>% with(data=., exp = glm(METHOn ~ nephgp+age+sex+anirt+HBINI+CREATINI+ALBINI, family = "binomial")) %>% pool() %>% summary()

modele.4.gr <- mice.tab.hd.gr %>% with(data=., exp = glm(METHOn ~ nephgp+age+sex+anirt+MARCHn, family = "binomial")) %>% pool() %>% summary()
detach(package:mice)
```
...
modele.0.gr %>%
  ...
  mutate(est = exp(est) %>% round(2),
         ...
         rowname = c("Intercept","Diabète","IgA","GNC 1","GNC 2")) %>%
  kable()
```
...
library(mice)
modele.0.dp <- mice.tab.hd.dp %>% with(data=., exp = glm(METHOn ~ nephgp, family = "binomial")) %>% pool() %>% summary()
...
coxph(Surv(fgstart, fgstop, fgstatus)~METHOn, data = x, weight = fgwt) %>%
  cox.display() %>%
  as.data.frame() %>%
  select_(2, 3) %>%
  kable()
...
coxph(Surv(kmdelaidc, kmdc)~gr_delai_greffe, data = x) %>%
  cox.display() %>% 
  as.data.frame() %>%
  select_(2, 3) %>%
  kable()
...
iga %>%
  ...
  tmerge(., ., id, enum = cumtdc(tstart)) %>% 
  mutate(event = ifelse(ajchangement==0, 2*kmdc, 1) %>% factor(0:2, labels = c("censure","changement","deces"))) %>% 
  coxph(Surv(tstart, tstop, event == "deces")~gr_delai_greffe+cluster(id), data = .)
```

## Courbe de survie globale (HD, DP, et greffe pré-emptive) comparant la néphropathie à IgA avec les 4 groupes

> Tu souhaites quoi comme comparaison ? Comme la première figure ou les 5 séparées ?
...
irc.etude %>%
  ...
  survfit(Surv(fgstart/365.25, fgstop/365.25, fgstatus)~nephgp, data = .) %>% 
...
text(5, 0.4, "Médiane diabète = 6.41 ans")
