...
iga %>%
  ...
  coxph(Surv(fgstart, fgstop, fgstatus) ~ METHOn, data = ., weight = fgwt) %>%
  summary()
...
iga %>%
  ...
  mutate(event = ifelse(ajchangement==0, 2*kmdc, 1) %>% factor(0:2, labels = c("censure","changement","deces"))) %>% print
  coxph(Surv(tstart, tstop, event=="deces")~METHOn+cluster(id), data = .) %>%
  summary()
```
